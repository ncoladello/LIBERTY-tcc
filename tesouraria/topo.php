...
<div id="box_topo">
 ...
 <div id="logo">
  <a href="index.php"><img border="0" src="../img/logo.png" width="250" /></a>
 </div><!-- logo -->
 ...
 <?php if(isset($_POST['search'])){
  ...
  if($key == ''){
	  ...
	  $sql_2 = "SELECT * FROM clientes WHERE code = '$key' OR nome = '$key' OR cpf = '$key'";
	  $result_2 = mysqli_query($conexao, $sql_2);
	  if(mysqli_num_rows($result_2) >= 1){
		  while($res_2 = mysqli_fetch_assoc($result_2)){	
		  	$code_cliente = $res_2['code'];
	   echo "<script language='javascript'>window.location='mostrar_cliente.php?matricula=$code_cliente';</script>";	
		  }
	  }else{
