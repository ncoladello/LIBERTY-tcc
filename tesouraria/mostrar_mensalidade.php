  ...
  <?php if (@$_GET['status'] == 'mostra_fatura') { ?>
    ...
    <div id="box">
      ...
      while ($res_1 = mysqli_fetch_assoc($result_1)) {
        ...
        $sql_2 = "SELECT * FROM clientes WHERE code = '$matricula'";
      ...
      ?>
          <table width="950" border="0">
            ...
            <?php if ($res_1['status'] != 'Pagamento Confirmado') { ?>
              <tr>
                <td colspan="4">| <a rel="superbox[iframe][270x200]" href="mostrar_mensalidade.php?mensalidade=<?php echo $mensalidade; ?>&status=baixa_mensalidade&valor=<?php echo $res_1['valor']; ?>&matricula=<?php echo $matricula; ?>">Informar pagamento</a> | <a rel="superbox[iframe][270x200]" href="mostrar_mensalidade.php?mensalidade=<?php echo $mensalidade; ?>&status=reajustar_mensalidade&valor=<?php echo $res_1['valor']; ?>">Reajustar valor de cobrança</a> | </td>
              </tr>
            <?php } ?>
          </table>
      <?php }
      ...
      <ul>
        ...
        <li>Não se esqueça que antes de você reajustar a mensalidade, você deve ter autorização da direção para executar tal função.</li>
        <li>Atenção, se você enviar um alerta de cobrança ao cliente você será a responsável por tal atitudade.</li>
        <li>Caso tenha qualquer dúvida entre em contato com a direção.</li>
      </ul>
    </div><!-- box -->
  <?php } ?>
  ...
  <?php if ($_GET['status'] == 'baixa_mensalidade') { ?>
    ...
    <div id="box_baixa">
      ...
      <?php if (isset($_POST['confirm'])) {
        ...
        $sql_2 = "INSERT INTO fluxo_de_caixa (status, tipo, d, m, a, date_completo, date, codigo, descricao, valor, form_pag) VALUES ('Ativo', 'CRÉDITO', '$d', '$m', '$a', '$date', '$dia', 'mensalidade=$code', 'Mensalidade do cliente: $matricula Competencia: $m/$a', '$valor', '$forma_de_pagamento')";
  ...
  <?php if ($_GET['status'] == 'reajustar_mensalidade') { ?>
    ...
    <div id="box_baixa">
      <?php if (isset($_POST['confirm'])) {
        ...
        $sql_2 = "INSERT INTO mural_coordenacao (date, status, curso, titulo) VALUES ('$date', 'Ativo', 'Não informado', 'A mensalidade de um cliente foi reajustada!')";
        mysqli_query($conexao, $sql_2);
